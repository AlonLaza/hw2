...
public class FvmFacade {
    ...
    public ProgramGraph<String, String> programGraphFromNanoPromela(String filename) throws Exception {
        ...
        for (PGTransition<String, String> transition : transitions) {
            if (!transition.getFrom().equals(TRUE_COND)) pgGraph.addTransition(transition);
            else pgGraph.addTransition(transition);
        }
        return pgGraph;
    }
    ...
    public List<PGTransition<String, String>> generatePGTransitionsFromSimpleStmnt(NanoPromelaParser.StmtContext sc, boolean cameFromIf, List<PGTransition<String, String>> allTransitions) {
        if (isSimpleStatement(sc) && !cameFromIf) {
            PGTransition<String, String> transition = new PGTransition<>(sc.getText().toString(), TRUE_COND, sc.getText().toString(), TRUE_COND);
            allTransitions.add(transition);
        } else if (isSimpleStatement(sc) && cameFromIf) {
            PGTransition<String, String> transition = new PGTransition<>(TRUE_COND, TRUE_COND, sc.getText().toString(), TRUE_COND);
            allTransitions.add(transition);
        }
        return allTransitions;
    }

    public void execSingleIfElseCommand(boolean ifCond, String type, String Succ, String fail, PGTransition<String, String> transition) {
        if (ifCond) {
            switch (type) {
                case "from": {
    ...
    public void addingTransitionsByTypeOfStmt(List<PGTransition<String, String>> allTransitions, ParserRuleContext stmt, String type, boolean CameFromIf) {
        switch (type) {
            case "if": {
                ...
                for (NanoPromelaParser.OptionContext oc : ifSStatement.option()) {
                    ...
                    for (PGTransition<String, String> transition : nestedTransitions) {
                        ...
                        boolean toPutIfCondBefore = transition.getFrom().equals(ifSStatement.getText().toString()) || transition.getFrom().equals(TRUE_COND);
                        if (toPutIfCondBefore) {
                            if (transition.getCondition().equals(TRUE_COND))
                                transition.setCondition("(" + oc.boolexpr().getText().toString() + ")");
                            else
                                transition.setCondition("(" + oc.boolexpr().getText().toString() + ") && (" + transition.getCondition() + ")");
                        }
                        allTransitions.add(transition);
                    }
                }
            ...
            case "do": {
                ...
                PGTransition<String, String> condcondIfExit = new PGTransition<>(TRUE_COND, TRUE_COND, TRUE_COND, TRUE_COND);
                ...
                for (NanoPromelaParser.OptionContext oc : doStatement.option()) {
                    ...
                    for (PGTransition<String, String> transition : nestedTransitions) {
                        ...
                        if (CameFromIf)
                            if (!transition.getFrom().equals(TRUE_COND)) transition.setFrom(transFrom + ";" + loopTag);
                            else
                                execSingleIfElseCommand(transition.getFrom().equals(TRUE_COND), "from", loopTag, transFrom + ";" + loopTag, transition);

                        if (transition.getFrom().equals(loopTag) || transition.getFrom().equals(TRUE_COND)) {
                            String transCondition = transition.getCondition();
                            if (transCondition.equals(TRUE_COND)) transition.setCondition("(" + cond + ")");
                            else transition.setCondition("(" + cond + ") && (" + transCondition + ")");
                        }
                        execSingleIfElseCommand(transition.getTo().equals(TRUE_COND), "to", loopTag, transition.getTo() + ";" + loopTag, transition);
                    }
                    execSingleIfElseCommand(condcondIfExit.getCondition().equals(TRUE_COND), "condition", "(" + cond + ")", condcondIfExit.getCondition() + "||(" + cond + ")", condcondIfExit);
                    ...
                    for (PGTransition<String, String> nesTransition : nestedTransitions) {
                        if (nesTransition.getFrom().equals(TRUE_COND)) {
                            PGTransition<String, String> newTransition = new PGTransition<>(loopTag, nesTransition.getCondition(), nesTransition.getAction(), nesTransition.getTo());
                ...
                if (CameFromIf) allTransitions.add(condcondIfExit);
    ...
    public List<PGTransition<String, String>> generatePGTransitionsFromIfStmnt(NanoPromelaParser.StmtContext sc, boolean CameFromIf, List<PGTransition<String, String>> allTransitions) {
    ...
    public List<PGTransition<String, String>> generatePGTransitionsFromDoStmnt(NanoPromelaParser.StmtContext sc, boolean CameFromIf, List<PGTransition<String, String>> allTransitions) {
    ...
    public List<PGTransition<String, String>> generatePGTransitionsFromCompoundStmnt(NanoPromelaParser.StmtContext sc, boolean CameFromIf, List<PGTransition<String, String>> allTransitions) {
        ...
        if (stmts != null) {
            ...
            for (PGTransition<String, String> transition : nestedTransitions) {
                if (CameFromIf)
                    if (!transition.getFrom().equals(TRUE_COND)) transition.setFrom(transition.getFrom() + ";" + nextStmt);
                    else transition.setFrom(transition.getFrom() + ";" + nextStmt);
                execSingleIfElseCommand(transition.getTo().equals(TRUE_COND), "to", nextStmt, transition.getTo() + ";" + nextStmt, transition);
            }
            allTransitions.addAll(nestedTransitions);
    ...
    public List<PGTransition<String, String>> generatePGTransitions(NanoPromelaParser.StmtContext sc, boolean cameFromIf) {
