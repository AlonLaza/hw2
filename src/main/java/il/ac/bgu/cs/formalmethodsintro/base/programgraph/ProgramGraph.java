...
public class ProgramGraph<L, A> {
	 ...
	 * {@code asList("x := 15", "y:=9")} says that the initial value of x is 15 and
	 * that the initial value of y is 9.
	 * <p>
	 * Note that this method can be called several times with different parameters
	 * to allow for nondeterministic initialization.
	 *
	 ...
	 * @param isInitial whether {@code location} should be an initial location in
	 *                  {@code this}.
	 * @throws IllegalArgumentException, if {@code location} is not a location in
	 *                                   {@code this}.
	 */
	...
	public String toString() {
		return String.format("[ProgamGraph name:%s locations:%d transitions:%d]", getName(), getLocations().size(),
				getTransitions().size());
	}
